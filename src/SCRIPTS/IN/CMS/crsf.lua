...
local function crsfDisplayPortCmd(cmd, data)
    local payloadOut = { CONST.address.inav, CONST.address.transmitter, cmd }
    if data ~= nil then
        for i = 1, #(data) do
...
protocol.cms.poll = function()
    ...
    if (data ~= nil) and (#data > 2) then
        if (frameType == CONST.frameType.displayPort) and (data[CONST.frame.destination] == CONST.address.transmitter) and (data[CONST.frame.source] == CONST.address.inav) then
