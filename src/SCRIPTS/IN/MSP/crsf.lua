...
protocol.mspSend = function(payload)
    local payloadOut = { CRSF_ADDRESS_inav, CRSF_ADDRESS_RADIO_TRANSMITTER }
    for i=1, #(payload) do
...
protocol.mspPoll = function()
    while true do
        ...
        if cmd == CRSF_FRAMETYPE_MSP_RESP and data[1] == CRSF_ADDRESS_RADIO_TRANSMITTER and data[2] == CRSF_ADDRESS_inav then
            local mspData = {}
            for i = 3, #data do
                mspData[i - 2] = data[i]
            end
